...
const getFolderName = (rootfolder) => {
  const configPath = path.join(rootfolder, "urban-advocacy/hugo.toml");
...
const setupProject = () => {
  ...
  if (!fs.existsSync(path.join(rootfolder, "themes"))) {
    ...
    [
      {
        filepath: path.join(rootfolder, "urban-advocacy/hugo.toml"),
        regex: /^.*theme\s*=\s*("[^"\]]+"|\S+)/m,
      },
      {
        filepath: path.join(
          rootfolder,
          "urban-advocacy/config/_default/module.toml",
        ),
        regex: /\[\[imports\]\]\s*\r?\npath = "([^"]+)"/,
      },
    ...
    const urbanAdvocacySite = path.join(rootfolder, "urban-advocacy");
    iterateFilesAndFolders(urbanAdvocacySite, { destinationRoot: rootfolder });
    deleteFolder(urbanAdvocacySite);
  }
};
