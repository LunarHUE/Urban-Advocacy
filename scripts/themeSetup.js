...
const setupTheme = () => {
  ...
  if (!fs.existsSync(path.join(rootFolder, "urban-advocacy"))) {
    ...
    const folder = createNewfolder(rootFolder, "urban-advocacy");

    fs.readdirSync(rootFolder, { withFileTypes: true }).forEach((file) => {
      if (includesFiles.includes(file.name)) {
        if (file.isDirectory()) {
          const destination = path.join(rootFolder, "urban-advocacy", file.name);
